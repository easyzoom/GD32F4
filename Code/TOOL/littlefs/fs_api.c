...
uint8_t gd32_flash_read_byte(uint32_t addr)
{
    return *(volatile uint8_t*)addr; 
}


void gd32_flash_write_byte(uint32_t writeaddr, uint8_t *pbuffer, uint32_t num)   
{
...
int32_t fs_api_flash_read( const struct lfs_config *c, lfs_block_t block,  lfs_off_t off, void *buffer, lfs_size_t size )
{
    ...
    {
        temp[i] = gd32_flash_read_byte(addr);
        addr++;
    }
                             ...
                             lfs_size_t size )
{
    ...
    addr = GD32_FLASH_FLLESYS_START_BASE + c->block_size * block + off;
    fmc_unlock();
    gd32_flash_write_byte(addr, (uint8_t *)buffer, size);
    fmc_lock();
