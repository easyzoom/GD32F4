...
#define LOG_BUFFER_SIZE 2048         /**< log输出缓冲大小 */
...
#define LOG_TIME_STAMP xTaskGetTickCount() /**< 设置获取系统时间戳 */
...
#define ERROR_TEXT CSI(31) \
"E(%d) %s:" CSI(39) /**< 错误标签 */
#define WARNING_TEXT CSI(33) \
"W(%d) %s:" CSI(39) /**< 警告标签 */
#define INFO_TEXT CSI(32) \
"I(%d) %s:" CSI(39) /**< 信息标签 */
#define DEBUG_TEXT CSI(34) \
"D(%d) %s:" CSI(39) /**< 调试标签 */
#define VERBOSE_TEXT CSI(36) \
"V(%d) %s:" CSI(39) /**< 冗余信息标签 */
#else
...
typedef struct
{
        void (*write)(char *, short len); /**< 写buffer */
        char active;                      /**< 是否激活 */
        LogLevel level;                   /**< 日志级别 */
} Log;

extern char *logLevelText[];
 ...
 * @param json json
 */
#define logJsonDebug(desc, json)                                    \
        do                                                          \
        {                                                           \
                if (LOG_ENABLE)                                     \
                {                                                   \
                        logDebug(desc);                             \
                        logWriteJson(LOG_ALL_OBJ, LOG_DEBUG, json); \
                }                                                   \
        } while (0)
